...
function Childlookup() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      if (mounted) {
        const response = await fetchChildLookup(params.id);
        ...
        if (response.status === 200) {
          if (response.data.status === "E") {
            ...
            setState((state) => {
              return {
                ...state,
                data: response.data.lookup_details,
                skeletonLoading: false,
              };
            });
            const datas = dataSlice({
      ...
      <td justifycontent="right" align="right">
        <Menu shadow="sm" size="xs">
          ...
          <Menu.Dropdown>
            ...
            <Menu.Item
              onClick={() => handleChild(row.parentid)}
